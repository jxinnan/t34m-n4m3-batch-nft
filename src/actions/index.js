...
export const cartNFT = ({ Tezos, cartData }) => {
	return async (dispatch) => {
		try {
			const contract = await Tezos.wallet.at(config.contractAddress);
			...
			for (let i = 0; i < cartData.length; i++) {
				calls.push({kind: OpKind.TRANSACTION, ...contract.methods.collect(cartData[i].token_id).toTransferParams({ amount: Number(cartData[i].amount), mutez: true })});
				// batch = await batch.withTransfer({to: cartData[i].holder, amount: cartData[i].amount, mutez: true, fee: true});
			}

			let batch = await Tezos.wallet.batch(calls);
